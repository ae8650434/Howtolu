...
app.get("/:mail", function (req, res) {
  ...
  var updatesql = "UPDATE email_random SET random = ? WHERE mid = ?";
          ...
          if (err) {
            ...
            DB.query(msql, [mail], (err, result) => {
              if (err) {
                ...
                if (result.length == 0) {
                  DB.query(insertmsql, [data[0].mid, mail, random], (err, s) => {
                    if (err) {
                      console.log("X");
                      return res.status(400).send("插入失敗");
                    } else {
                      console.log("O");
                      // req.session.AAA = data[0]
                      // console.log('third',req.session)
                      return res.status(200).json({random: random});
                    }
                  });
                } else {
                  DB.query(updatesql, [random, data[0].mid], (err, q) => {
                    if (err) {
                      console.log("X");
                      ...
                      // console.log('third',req.sessionID)
                      return res.status(200).json({random: random});
                    }
                  });
                }
