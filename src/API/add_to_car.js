...
function insertDataToDB(data) {
    ...
    var sql = 'INSERT INTO `car`(`mid`, `pid`, `fid`, `c_day`, `use_date`, `return_date`, `quantity`, p_price, f_price) VALUES (?,?,?,?,?,?,?,?,?)';
    DB.query(sql, [data.mid, data.pid, data.fid, 3, use_date, return_date, data.quantity, data.p_price, data.f_price], (err, result) => {
        if (err) {
